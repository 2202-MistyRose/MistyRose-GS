...
export const clearCart = createAsyncThunk("cart/increment", async (userId) => {
  try {
    const { data: deleted } = await axios.delete(`/api/users/${userId}/cart`);
    return deleted;
  } catch (err) {
...
export const checkout = createAsyncThunk("cart/checkout", async (info) => {
  try {
  ...
  extraReducers: {
    ...
    [removeFromCart.fulfilled]: (state, action) => {
      state.cart = state.cart.filter(
        (item) => item.productId != action.meta.arg.item.productId
      );
      state.status = "success";
    },
    ...
    [updateQuantity.fulfilled]: (state, action) => {
      state.cart = state.cart.reduce((accum, current) => {
        if (current.productId === action.meta.arg.item.productId) {
          if (action.meta.arg.item.quantity > 0) {
            // if it becomes 0 don't return it
